...
def start_payment(request):
    ...
    # these credentials will be passed to merchant order processor to verify the business account
    param_dict = {
        'MID': env('MERCHANTID'),
        ...
        # this is the url of handlepayment function, merchant will send a POST request to the fuction associated with this CALLBACK_URL
    }
...
def handlepayment(request):
    checksum = ""
    # the request.POST is coming from merchant
    form = request.POST
    ...
    for i in form.keys():
        ...
        if i == 'CHECKSUMHASH':
            # 'CHECKSUMHASH' is coming from merchant and we will assign it to checksum variable to verify our paymant
            checksum = form[i]
    ...
    # we will verify the payment using our merchant key and the checksum that we are getting from merchant request.POST
    ...
    if verify:
        if response_dict['RESPCODE'] == '01':
            ...
            return render(request, 'merchant/paymentstatus.html', {'response': response_dict})
        else:
